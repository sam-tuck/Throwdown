...
router.post("/update", function (req, res) {
  gameservice.updateGameAsync(req).then((game) => {
    res.send(game[0]);
  });
});

router.post("/delete", function (req, res) {
  gameservice.deleteGameAsync(req).then((game) => {
    res.send(game[0]);
  });
});
...
router.post("/changedeetsup", function (req, res) {
  gameservice.changeGameUpAsync(req).then((game) => {
    res.send(game[0]);
  });
});

router.post("/changedeetsdown", function (req, res) {
  gameservice.changeGameDownAsync(req).then((game) => {
    res.send(game[0]);
  });
});
